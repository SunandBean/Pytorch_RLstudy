...
행동가치 함수를 표형식 표현으로 구현

- 행 : 상태 s, 열 : 행동 a 로 행동가치 함수 Q(s,a)를 구현

- 행동가치의 초기 값은 난수로 부여 (알 수 없기 때문)

**가치반복 알고리즘**

- Exploitation과 Exploration을 적절히 섞어서 행동을 선택해야한다.
  - Exploitation(이용) : 현재 행동가치 함수의 최댓값을 이용해 행동을 결정하는 방법
  - Exploration(탐색)무작위로 행동을 선택하는 방법



### 행동가치 함수 Q(s,a)를 Sarsa 알고리즘으로 수정

만약 행동가치 함수가 제대로 되어있다면 아래 식이 성립
$$
Q(s_t,a_t)=R_{t+1}+\gamma Q(s_{t+1},a_{t+1})\\\
$$
학습 중에는 행동가치 함수가 정확하지 않기 때문에 그에 따른 오차를 아래와 같이 설정
$$
TD\ error=R_{t+1}+\gamma Q(s_{t+1},a_{t+1})-Q(s_t,a_t)\\
TD \ error = 0 \ -> \ 정확한\ 행동가치\ 함수\ 학습!
$$
따라서 스텝마다 Q를 학습하기 위해서는 아래와 같은 식을 이용하여 Q를 수정
$$
Q(s_t,a_t) =Q(s_t,a_t)+\eta*(R_{t+1}+\gamma Q(s_{t+1},a_{t+1}) - Q(s_t,a_t))\\
\eta =학습률
$$
위와 같이 함수를 수정하는 알고리즘을 Sarsa라고 부른다

- 이유: 학습에 사용하는 요소가 현재 상태와 행동(s,a), 즉각 보상(R), 다음 단계의 상태와 행동(s,a)를 조합하기 때문
...
바뀐 부분
$$
\gamma Q(s_{t+1},a_{t+1}) \ -> \ \gamma max_aQ(s_{t+1},a)
$$

- 다음에 취할 행동을 구해서 행동가치 함수 수정(Sarsa) 
  - Q를 수정할 때 다음 상태에서의 행동까지 필요하므로 정책에 의존적 => 온-폴리시
- -> 다음 상태에서 행동가치 함수값이 가장 큰 것을 사용하여 행동가치 함수 수정
  - Q를 수정할 때 정책에 의존하지 않음 => 오프-폴리시
  - e-greedy 알고리즘에 의해 생기는 무작위 행동이 수정식에 포함되지 않아 Sarsa보다 빨리 수렴